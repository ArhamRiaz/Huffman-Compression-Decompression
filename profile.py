...
def profile(infile, number):
    ...
    if cmp(infile, decompressed):
        print("SUCCESS! Decompressed file and original file are binary same.")
        print("---- Stats ----")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Profiler for A2 Huffman Encoding by Aaron Goidel",
    ...
    parser.add_argument(
        "-f",
        "--file",
        help="path to the target file, defaults to 'files/parrot.bmp'",
        default="Homer-Iliad.txt",
    )
    parser.add_argument(
        "-n",
        "--number",
